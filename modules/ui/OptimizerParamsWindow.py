...
class OptimizerParamsWindow(ctk.CTkToplevel):
    ...
    def create_dynamic_ui(
        ...
        KEY_DETAIL_MAP = {
            ...
            'evap_build_min': {'title': 'Evap Build Threshold', 'tooltip': 'Hypothesis-infancy guard: the dissipation only fires on coordinates whose accumulated velocity has reached this many mantissa units. Default 128 = one s_slow LSB = one deploy-weight tick -- the first size at which a hypothesis can commit. Below it the velocity is invisible at deploy by construction, so friction there cannot protect anything; it can only kill nascent hypotheses before the coherence meter can judge them. Noise random-walks up to the threshold, is judged and drained there, and equilibrates below one deploy tick. 0 = legacy (dissipate at any size).', 'type': 'float'},
            ...
            'autotune_gamma_snr': {'title': 'Gamma-SNR Dissipation', 'tooltip': 'Timestep-SNR-modulated dissipation: per step kappa is scaled by mean(max(1, snr/knee)) over the batch (capped at lam=1). Plays the role of the min-SNR-gamma LOSS weight on the dissipation side -- the high-SNR timesteps min-SNR would down-weight get proportionally more friction instead, with the overall strength set by the autotuned base rather than a hand gamma. Pair with Loss Weight Function = CONSTANT (do not double up with MIN_SNR_GAMMA). Typical knee 5. Probe windows stay unmodulated. Empty = off.', 'type': 'float'},

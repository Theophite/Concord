...
class OptimizerParamsWindow(ctk.CTkToplevel):
    ...
    def create_dynamic_ui(
        ...
        KEY_DETAIL_MAP = {
            ...
            'gf_trust_delta_sq': {'title': 'Trust Region (delta^2)', 'tooltip': 'delta^2 in the step denominator g / sqrt(Var + delta^2 * v_hat). 1 = v_hat IS the denominator (rank-1 Adam, the validated winner). Lower loosens the floor (implied step bounded by ~1/delta); 0 removes the term entirely, leaving Step Cap as the only bound (legacy mode). Note this term doubles as the Adam-style preconditioning, not just a safety rail. Baked at CUDA-graph capture -- fixed for the run.', 'type': 'float'},
            ...
            'autotune_servo': {'title': 'Per-Layer Dissipation Servo', 'tooltip': 'Per-layer, table-free dissipation autotuner -- the replacement for the Autotune Table. Seeds each UNet layer\'s kappa from Dissipation (lam/lr) and climbs it ONE-SIDED once per epoch, but only on layers still EARNING: per-layer boil (coherent-waste fraction) below the Servo Boil Ceiling AND per-layer memgap (first-order deploy-loss) still shrinking vs last epoch. One-sided (never lowers) avoids the chase-your-own-success divergence a coherence setpoint has, since friction RAISES coherence. Needs Dissipation > 0 (the seed). Off = use the fixed Dissipation (or the Autotune Table if set). Honours the Gated Momentum pair above.', 'type': 'bool'},
            'autotune_climb_rate': {'title': 'Servo Climb Rate', 'tooltip': 'Per-epoch one-sided multiplier step for the servo: an earning layer\'s kappa goes kappa *= (1 + this) each epoch, capped at the linear-stability ceiling (lr*kappa < ~1.9). Default 0.5. No effect unless Per-Layer Dissipation Servo is on.', 'type': 'float'},
            'autotune_boil_ceiling': {'title': 'Servo Boil Ceiling', 'tooltip': 'The servo stops climbing a layer once its per-epoch boil -- the fraction of evaporated energy that was drift-aligned (learned signal) -- reaches this. ~0 means the gate eats only incoherent noise (safe to add friction); a rise means friction is starting to eat signal. Default 0.05. No effect unless Per-Layer Dissipation Servo is on.', 'type': 'float'},

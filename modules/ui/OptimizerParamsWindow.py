...
class OptimizerParamsWindow(ctk.CTkToplevel):
    ...
    def create_dynamic_ui(
        ...
        KEY_DETAIL_MAP = {
            ...
            'dissipation': {'title': 'Dissipation (lam)', 'tooltip': 'Dimensionless dissipation lam = lr * kappa, the physical friction knob (per-step friction fraction). When set it OVERRIDES GF Consolidation with lam/lr, so the same value means the same friction at any learning rate. nanoGPT winner 0.025 (= kappa 333 at lr 7.5e-5); CPU noisy-regime optimum up to 0.4; must stay below 2 (linear stability). Empty = off (use GF Consolidation directly).', 'type': 'float'},
            ...
            'autotune_beta1_on': {'title': 'Autotune Beta1 On', 'tooltip': 'Coherence-gated momentum, probe-selected: this beta1 is committed iff the probe coherence clears the threshold -- momentum only on streams the gate reads as clean. 0 = disabled (default; beta1 > 0 LOSES under label noise).', 'type': 'float'},
            'autotune_beta1_coh': {'title': 'Autotune Beta1 Threshold', 'tooltip': 'Probe-coherence threshold for committing Autotune Beta1 On. Calibrated in probe-window coherence units (MNIST default 0.35).', 'type': 'float'},

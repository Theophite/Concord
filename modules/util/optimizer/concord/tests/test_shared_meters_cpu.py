"""CPU unit tests for the SHARED dissipation/telemetry infrastructure (no GPU, no Triton).

Covers the controller-and-kernel-module pieces that survive independent of any
dissipation controller (the hunting servos are gone; the NoiseScaleSeeder is the only
controller, tested in test_nsr_seeder_cpu.py):

  9    the min_leak survival clamp (kernel formula mirror + module global + pick-through)
  10   the SHARED memorization-gap meter: buffer machinery, the cold-start drain on the
       first read (self-test/warmup lump discarded), and the controller's sign flip
  11   the soft, size-proportional build gate (evap_build_min)
  12   the telescope-fill dissipation ramp (and its composition with the gamma-SNR hook)
  13   the SHARED boil meter (false-kill audit): energy decomposition, the cold-start
       drain, boil/waste fractions, the lag-tax signature
  14   the telescope epoch window (alpha_v pinned to the revisit period; C* re-derived)
  15   embedding-core registration (friction at the embedding lr; deploy bridge)
  16   embedding anchor mode (position in v_slow, frozen leak, deploy direction/norm)
  17   embedding release delay (divot), auto-drive calibration, window posterior math,
       token-only caption dropout, and the winner_step sigma-global hazard

Run with the OneTrainer venv python:
  venv/Scripts/python.exe modules/util/optimizer/concord/tests/test_shared_meters_cpu.py
"""
import sys
...
# -- gamma-SNR hook rig (shared by section 12's composition check) -------------
def snr_hook_rig(knee, base_committed, lr=7.5e-5, tuner=True, gf_consol=333.0):
    rig = SimpleNamespace(
        ...
        te_layers=[], te_groups=[],          # on_timesteps modulates winner-TE groups too
        _snr_mod_announced=True, _LAM_MOD_CAP=ConcordController._LAM_MOD_CAP,
        _current_fill_ramp=1.0,
        _csnr_step=lambda *a, **k: None)     # CSNR meter is orthogonal to the hook under test
    return rig
...
gap0 = ConcordController.read_memorization_gap(rig10)
check("10c first SHARED read is the cold-start drain (self-test/warmup lump discarded, buffer zeroed)",
      gap0 == 0.0 and float(buf[0]) == 0.0)
buf += -0.5                          # a real training window after the drain
gap = ConcordController.read_memorization_gap(rig10)
check("10c2 controller flips sign after the drain: deploy reads HIGHER than live",
...
boil13c, waste13c = ConcordController.read_flow_audit(rig13)
check("13c0 first SHARED read is the cold-start drain (self-test/warmup lump discarded, buffer zeroed)",
      boil13c is None and waste13c is None and float(bbuf.sum()) == 0.0)
bbuf[0] += 1.0; bbuf[1] += 5.0; bbuf[2] += 15.0   # a real training window after the drain
...
def ew_rig(flag=True, av=0.001):
    return SimpleNamespace(
        # apply_epoch_window also drives the v-hat beta2 window and the chase-alpha window; this test
        # isolates the TELESCOPE retiming, so keep both no-ops: v-hat block harmless (no layer tracks
        # adafactor_v), chase off (chase_epoch_window False + chase_alpha 0 -> alpha stays 0.1, so 14b's
        # C* is re-derived from the ORIGINAL alpha).
        config=SimpleNamespace(telescope_epoch_window=flag, alpha_v_fast=av,
                               beta2_epoch_window=True, beta2=0.999,
                               vhat_warmstart=False, bias_correct_v=False,
                               chase_epoch_window=False, chase_alpha=0.0),
        emb_cores=[], emb_delay_epochs=0.0, emb_delay_steps=0, steps_per_epoch=0.0,
        te_layers=[], te_groups=[],          # apply_epoch_window telescopes winner TEs too
...
rig17 = SimpleNamespace(config=SimpleNamespace(telescope_epoch_window=False, alpha_v_fast=0.001,
                            beta2_epoch_window=True, beta2=0.999, vhat_warmstart=False,
                            bias_correct_v=False, chase_epoch_window=False, chase_alpha=0.0),
                        emb_cores=[object()], emb_delay_epochs=1.0,
                        emb_delay_steps=0, steps_per_epoch=0.0, layers=[], te_layers=[])

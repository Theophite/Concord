...
if __name__ == "__main__":
    ...
    torch.manual_seed(0)
    D = 64
    # Frozen vocab with a long junk tail: most tokens norm ~7, a chunk near 0 and
    # a few huge -> mean is skewed, median is robust.
    base = torch.randn(4000, D, device=dev)
    base[:1200] *= 0.15                     # low-information tokens (small norm)
    base[3990:] *= 9.0                      # a few outliers (huge norm)
    med = ConcordNewTokenEmbedding.vocab_median_norm(base)
    mean = base.norm(dim=1).mean().item()
    print(f"[vocab] median row-norm {med:.2f}  vs  mean {mean:.2f}  "
          f"(median ignores the junk tail)")

    # 4 new concept tokens; a target DIRECTION to learn (unit-ish), large magnitude
    # to tempt norm blow-up.
    ntok = 4
    tgt_dir = torch.randn(ntok, D, device=dev)
    tgt = tgt_dir / tgt_dir.norm(dim=1, keepdim=True) * 30.0
    ids = torch.arange(ntok, device=dev)

    emb = ConcordNewTokenEmbedding(ntok, D, device=dev, lr=5e-2, target_norm=med)
    emb.init_tokens()
    print(f"[init] new-token deploy norm {emb.deploy_weight().norm(dim=1).mean():.2f} "
          f"(snapped to median {med:.2f})")
    for it in range(300):
        loss = F.mse_loss(emb(ids).float(), tgt)
        loss.backward()
    dep = emb.deploy_weight()
    cos = F.cosine_similarity(dep, tgt, dim=1).mean().item()
    print(f"[trained] deploy norm {dep.norm(dim=1).mean():.2f} (pinned to median "
          f"{med:.2f}) | live norm {emb.weight.norm(dim=1).mean():.2f} (s_fast-clamped) "
          f"| cos-to-target {cos:.3f}")
    print("-> learns the concept DIRECTION (cos high) while the shipped embedding "
          "stays at the typical-token norm; the clamp keeps the live weight bounded.")

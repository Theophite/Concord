> **HISTORICAL.** This was the repo's root README during the pre-packed-B era; the
> integration it describes (patch + `CONCORD_SGD`) predates the `concord-integration`
> branch. The current front door is the repo root `README.md`.
